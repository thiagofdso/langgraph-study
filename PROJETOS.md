...
- **Abordagem técnica**: estrutura modular alinhada aos demais agentes (`config`, `state`, `utils/`, `graph`, `cli`, `tests`). O grafo executa `validate_input` → `invoke_model` → `plan_tool_usage` → `execute_tools` → `invoke_model` (loop até não haver novas ferramentas) → `format_response`. O Gemini decide quando chamar a ferramenta via `tool_calls`; todas as chamadas são agrupadas em listas (`tool_plans`, `tool_calls`) e respondidas uma a uma pela calculadora endurecida (AST sandbox). O checkpointer `MemorySaver` preserva estado por `thread_id`, a CLI (`python -m agente_tool run`) executa pré-checagens de configuração e logs estruturados, e a suíte `pytest` cobre nodes (incluindo logging) e o fluxo completo com LLM stub.
